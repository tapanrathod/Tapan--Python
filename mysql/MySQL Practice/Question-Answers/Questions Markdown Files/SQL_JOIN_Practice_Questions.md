...
8.  Display all customers who purchased products from the "Laptop"
    category.
9.  Display customer email along with their purchased product.
10. Show order ID, customer name, and order amount sorted by highest
    amount.

## Part 2: INNER JOIN + Filtering

11. Display all customers whose order amount is between ₹10,000 and
    ₹70,000.
...
19. Display orders sorted first by category and then by amount
    descending.
...
Create a `projects(project_id, project_name, department_id, budget)`
table.
...
135. Display customers who purchased products from more than one
     category.
